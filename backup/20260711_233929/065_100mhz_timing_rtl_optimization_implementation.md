...
为保持简单的时序边界，full 状态本实现不使用同拍 pop 释放的空间接收 rename，最多
引入一个局部 backpressure bubble，但没有 payload compaction。lane1 offer 继续依赖
lane0 fire，ROB allocation、目标 IQ acceptance 和 buffer pop 仍是同一个原子事件。
