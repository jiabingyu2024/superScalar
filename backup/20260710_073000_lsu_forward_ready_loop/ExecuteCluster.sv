...
module CoreExecuteCluster (
    ...
    always_comb begin
        ...
        if (mem_issue_valid_i[0] &&
            mem_issue_uop_i[0].uop.is_load &&
            !mem_misaligned(mem_issue_uop_i[0].uop, result[INT_ISSUE_WIDTH])) begin
            load_query_valid_o = 1'b1;
            load_query_addr_o = result[INT_ISSUE_WIDTH];
            load_query_mask_o = load_mask(mem_issue_uop_i[0].uop, result[INT_ISSUE_WIDTH]);
        end
