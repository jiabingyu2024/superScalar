...
P0  完整/矩阵窗口计数 + 当前 RTL routed baseline
 ...
 -> routed top-path 分类
 -> P1R CompressedQueue payload/control 分离 + ROB/BPU/rename 路径定向修复
 -> P2B allocated DispatchBuffer（rename 路径命中时提前）
 -> P1B ISS/RRD/EXE 仅在 routed path 命中后重新评估
...
Vivado 综合、布局布线和 IP regeneration 本批次均未由本代理自动调用。随后用户侧导出了 commit `66454dd` 的 100 MHz routed report；它不含本节 P1A 的未提交 RTL，但可以决定后续结构优先级。

### 12.2 commit 66454dd 的 100 MHz routed 基线

报告来源：

```text
/mnt/e/Resources/03_competitions/26_03_jcs/2607round/superScalar/fpga/build/
  digital_twin_srcWithMext/exported_reports/
  commit_66454dd_100mhz_routed_20260711/
```
...
| 指标 | 值 |
| --- | ---: |
| CPU constraint | 10.000 ns / 100 MHz |
| CPU WNS / TNS | -4.275 ns / -126,470.969 ns |
| 最差 data path | 13.869 ns，logic 1.724 ns，route 12.145 ns |
| 近似现有 Fmax | 约 70.1 MHz |
| hold WHS / THS | +0.068 ns / 0 |
| no_clock / unconstrained internal | 0 / 0 |

对 `setup_top200.rpt` 的全部 600 个展开 path header 分类，而不是只看第一条：

| 路径关键词 | path 命中数 | 解释 |
| --- | ---: | --- |
| `u_bpu` | 107 | ROB/commit retirement 组合解包后驱动 BPU history/PHT 更新 |
| `u_rename` | 142 | sRAT/rename 映射跨 dispatch 到压缩 IQ payload/control |
| `u_mem_iq` | 128 | MEM IQ push/压缩和 payload 大 mux |
| `dram_adapter` | 20 | ROB/store commit 到 DRAM BRAM data/write path |
| `u_dcache` | 35 | commit/store/cache 路径 |
| `u_free_list` | 40 | rename/free allocation 网络 |
| `u_execute` / `u_prf` | **0** | top-600 中没有命中 |

最差单条是 `ROB head/retire -> BPU choice PHT`，其次是 `ROB head/retire -> DRAM BRAM DI`；大量约 -4.24 ns 路径是 `sRAT -> MEM IQ entry payload/control`。这与 058 的更早 MEM issue/PRF/AGU 路径不同，说明 060 的 request stage 已经移动了瓶颈。

因此 P1B 的 INT IQ/PRF/EXE stage cut 当前不满足进入条件。真正的下一批应先：

1. 把 `CoreCompressedQueue` 的 valid/count 异步 reset ownership 与宽 payload 寄存器分离，消除 payload `/S` 控制路径和 Synth 8-7137 类问题；
2. 将 allocated DispatchBuffer 提前，切断 sRAT/free/ROB allocation 到 IQ push/压缩的跨模块组合路径；
3. 分离 commit/ROB retire 与 BPU table update，避免宽 ROB entry 解包、索引和多表写控制同拍跨芯片布线；
4. 单独检查 committed store 到 DRAM 的 data/control 路径，不能用 INT stage cut 假装修复。

### 12.3 Phase 1B：INT RRD A/B 候选已拒绝并回退

为验证结构假设，曾实现两路 INT `IQ -> RRD -> PRF/EXE`，并从 RRD 导出 fixed-latency early-wakeup。备份位于：

```text
backup/20260711_122514_p1b_int_rrd/
```

调试过程中得到两条可复用结论：

- 直接把 packed `exec_complete_*` 反馈给 IQ 会形成 `MEM IQ -> PRF -> completion vector -> wakeup -> MEM IQ` 组合环；不能用 waiver 掩盖；
- INT early-wakeup 若广播给没有 RRD 的 MEM/MUL consumer，会在 producer 写 PRF 的同一边沿读取旧值。`rv32ui-p-st_ld` FST 在 1950 ps 明确显示 MEM 读取 PRD27/28 为 0/0，而同一边沿 producer 才写入 `0x80002000/0xffffffdd`。完整记录见 `docs/stage11_wave_debug.json` 的 `p1b_int_rrd_early_wakeup_data_mismatch` session。

修正为“early-wakeup 仅送 INT IQ，MEM/MUL 用 registered completion”后功能全过，但性能为：

| 项目 | P1A | P1B 候选 | 变化 |
| --- | ---: | ---: | ---: |
| `srcWithMext` 500k IPC | 0.882140 | 0.812654 | -7.88% |
| `srcWithMext` 500k commits | 441,070 | 406,327 | -34,743 |
| full `srcSmoke` cycles | 33,398,117 | 35,311,884 | +5.73% |
| full `srcSmoke` IPC | 0.920968 | 0.871055 | -5.42% |
| RV32UI/UM/MI | PASS | 40/40、8/8、4/4 PASS | 功能不变 |
| 500k difftest | PASS/TIMEOUT | 406,332 commits；37+8、fail 0 | 正常推进 |

一个额外 MEM RRD 试验的 500k IPC 只有 0.812150，没有恢复收益，已一并删除。

在不知道路径时，P1B 至少需要 Fmax 提升 `0.882140 / 0.812654 - 1 = 8.55%` 才能抵消 IPC 损失；新 routed report 又证明 top-600 没有 INT execute/PRF 路径，所以联合收益条件不成立。`ExecuteCluster.sv` 和 `CoreBackend.sv` 已逐字节恢复到 P1A 批次起点；回退后 `st_ld` PASS，500k 恢复 441,070 commits、IPC 0.882140、37+8、fail 0、8 read/miss。
