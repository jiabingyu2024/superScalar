...
module CoreFreeList #(
    ...
    PhyRegNumPath recover_mem [DEPTH-1:0];
    logic [COUNT_WIDTH-1:0] recover_count;
    ...
    always_comb begin
        ...
        for (int p = 0; p < PHY_REG_NUM; p = p + 1) begin
            ...
            for (int r = 1; r < LOGIC_REG_NUM; r = r + 1) begin
                if (recover_map_i[r] == PhyRegNumPath'(p)) begin
                    used_by_arch = 1'b1;
                end
            end
            if (!used_by_arch) begin
                recover_mem[recover_count] = PhyRegNumPath'(p);
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst || clear_i) begin
            ...
            rd_ptr_q <= '0;
            wr_ptr_q <= recover_count[PTR_WIDTH-1:0];
            count_q <= recover_count;
            for (int i = 0; i < DEPTH; i = i + 1) begin
                mem_q[i] <= recover_mem[i];
            end
        end else begin
            for (int j = 0; j < FREE_WIDTH; j = j + 1) begin
                if (free_fire[j]) begin
