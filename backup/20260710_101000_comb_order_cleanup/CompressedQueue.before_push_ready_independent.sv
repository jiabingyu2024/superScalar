...
module CoreCompressedQueue #(
    ...
    always_comb begin
        ...
        issue_remove = '0;
        issue_count = '0;
        issue_prefix_fire = 1'b1;
        for (int p = 0; p < ISSUE_PORTS; p = p + 1) begin
            issue_valid_o[p] = 1'b0;
            issue_uop_o[p] = '0;
            if (issue_prefix_fire && HEAD_ONLY) begin
            ...
            end else if (issue_prefix_fire) begin
                for (int e = 0; e < DEPTH; e = e + 1) begin
                    if (!issue_valid_o[p] &&
                        valid_q[e] &&
                        !selected_mask[e] &&
            ...
            issue_prefix_fire = issue_prefix_fire && issue_fire[p];
        end
