...
`endif
    ...
    logic [3:0]       wb_cand_valid;
    logic [`RF_BUS]   wb_cand_rd [0:3];
    logic [`DATA_BUS] wb_cand_data [0:3];
    logic             wbq_overflow;
    ...
    // Results are produced by two physically separate lanes but the mirrored
    // LUTRAM register file has one architectural write port.  Pending entries
    // are older than current C2 results; a same-cycle LS result is older than
    // the EX result because the LS sidecar holds the earlier instruction.
    always_comb begin : wb_ordered_gearbox
        integer cand_idx;
        integer keep_count;

        wb_cand_valid[0] = wbq0_valid;
        wb_cand_rd[0]    = wbq0_rd;
        wb_cand_data[0]  = wbq0_data;
        wb_cand_valid[1] = wbq1_valid;
        wb_cand_rd[1]    = wbq1_rd;
        wb_cand_data[1]  = wbq1_data;
        wb_cand_valid[2] = ls_wb_fire_c2;
        wb_cand_rd[2]    = ls_wb_rd_addr;
        wb_cand_data[2]  = ls_wb_data_c2;
        wb_cand_valid[3] = ex_wb_fire_c2;
        wb_cand_rd[3]    = rd_addr_c2;
        wb_cand_data[3]  = ex_wb_data_c2;
        ...
        for (cand_idx = 0; cand_idx < 4; cand_idx = cand_idx + 1) begin
            if (wb_cand_valid[cand_idx]) begin
                if (!wb_fire_c2) begin
                    wb_fire_c2    = 1'b1;
                    wb_rd_addr_c2 = wb_cand_rd[cand_idx];
                    wb_data_c2    = wb_cand_data[cand_idx];
                end else if (keep_count == 0) begin
                    wbq0_valid_next = 1'b1;
                    wbq0_rd_next    = wb_cand_rd[cand_idx];
                    wbq0_data_next  = wb_cand_data[cand_idx];
                    keep_count      = 1;
                end else if (keep_count == 1) begin
                    wbq1_valid_next = 1'b1;
                    wbq1_rd_next    = wb_cand_rd[cand_idx];
                    wbq1_data_next  = wb_cand_data[cand_idx];
                    keep_count      = 2;
                end else begin
                    wbq_overflow = 1'b1;
                end
...
`endif
    ...
    assign ls_operands_ready = (!ls_addr_pending || addr_dep_ex_match ||
                                addr_dep_ls_match || addr_dep_q1_match ||
                                addr_dep_q0_match) &&
                               (!ls_data_pending || data_dep_ex_match ||
                                data_dep_ls_match || data_dep_q1_match ||
                                data_dep_q0_match);
