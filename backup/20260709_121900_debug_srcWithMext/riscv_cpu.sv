...
`ifdef VERILATOR_TB
    always_ff @(posedge clk) begin
        if (!rst && perf_cycle > 64'd3000 && perf_cycle[19:0] == 20'd0) begin
            $display("DBG cycle=%0d state=%0d pc=%08x inst=%08x irom_addr=%08x irom_data=%08x dreq=%0b dwr=%0b daddr=%08x dresp=%0b drdata=%08x mtvec=%08x mepc=%08x mcause=%08x",
                     perf_cycle, state_q, pc_q, exec_inst_c, irom_addr, irom_data,
                     dmem_req_valid, dmem_req_write, dmem_req_addr,
                     dmem_resp_valid, dmem_resp_rdata,
                     csr_mtvec_q, csr_mepc_q, csr_mcause_q);
        end
    end
`endif
