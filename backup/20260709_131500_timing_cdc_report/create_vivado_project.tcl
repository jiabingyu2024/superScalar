...
proc write_cdc_constraint_file {script_path} {
    ...
    puts $fh {set sys_clk [get_clocks -quiet clk_out1_pll]}
    puts $fh {set cpu_clk [get_clocks -quiet clk_out2_pll]}
    puts $fh {if {[llength $sys_clk] == 0} {
    set sys_clk [get_clocks -quiet -of_objects [get_pins -hier -filter {NAME =~ */clkout1_buf/O}]]
}}
    puts $fh {if {[llength $cpu_clk] == 0} {
    set cpu_clk [get_clocks -quiet -of_objects [get_pins -hier -filter {NAME =~ */clkout2_buf/O}]]
}}
    puts $fh {if {[llength $sys_clk] > 0 && [llength $cpu_clk] > 0} {
