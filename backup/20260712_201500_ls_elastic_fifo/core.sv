...
`endif
    ...
    (* max_fanout = 64 *) logic rst_front_n_q;
    (* max_fanout = 64 *) logic rst_exec_n_q;
    (* max_fanout = 64 *) logic rst_ls_n_q;

    // The SoC already synchronizes reset release. Replicate it once per local
    // physical region so the top-level reset net only drives three flops.
    ...
    assign wbq_cold_stall = wbq1_valid;
...
`endif
    ...
    logic             data_dep_c1_d;
    logic             data_dep_ls_d;
    logic [`DATA_BUS] mem_addr_capture_d;
    logic [`DATA_BUS] store_data_capture_d;
    ...
    assign addr_dep_c1_d = valid_d && (mem_read_d || mem_write_d) &&
                           valid_c1 && reg_write_c1 &&
                           (rd_addr_c1 != '0) &&
                           (rd_addr_c1 == rs1_addr_d);
    assign addr_dep_ls_d = valid_d && (mem_read_d || mem_write_d) &&
                          valid_ls && mem_read_ls &&
                          (rd_addr_ls != '0) &&
                          (rd_addr_ls == rs1_addr_d);
    assign data_dep_c1_d = valid_d && mem_write_d &&
                           valid_c1 && reg_write_c1 &&
                           (rd_addr_c1 != '0) &&
                           (rd_addr_c1 == rs2_addr_d);
    assign data_dep_ls_d = valid_d && mem_write_d &&
                          valid_ls && mem_read_ls &&
                          (rd_addr_ls != '0) &&
                          (rd_addr_ls == rs2_addr_d);

    // A dependency on the current C1 producer terminates at the LS input
    // register. The following cycle starts from this local address/data rather
    // than broadcasting the C2/queue result through DCache and global stall.
    assign mem_addr_capture_d = addr_dep_c1_d ?
                                (alu_res_c1 + imm_d) : mem_addr_d;
    assign store_data_capture_d = data_dep_c1_d ?
                                  alu_res_c1 : store_data_d;
    assign addr_pending_d = addr_dep_ls_d && !addr_dep_c1_d;
    assign addr_dep_rd_d = rd_addr_ls;
    assign data_pending_d = data_dep_ls_d && !data_dep_c1_d;
    assign data_dep_rd_d = rd_addr_ls;
    ...
    reg_id_ls u_reg_id_ls (
        ...
        .i_hold          (ls_busy),
        .i_valid         (valid_d && (mem_read_d || mem_write_d) &&
                          !m_busy_c1 && !wbq_cold_stall),
        .i_mem_addr      (mem_addr_capture_d),
        .i_store_data    (store_data_capture_d),
        .i_offset        (imm_d),
    ...
    stage_ex u_stage_ex (
        .i_clk           (clk),
        .i_rst_n         (rst_exec_n_q),
        .i_flush_e       (1'b0),
        .i_stall_e       (c1_busy && !wbq_cold_stall),
        ...
        .i_func3         (func3_c1),
        .i_is_branch     (valid_c1 && is_branch_c1 && !wbq_cold_stall),
        .i_is_rs2_imm    (is_rs2_imm_c1),
        .i_inst_spec     ((valid_c1 && !wbq_cold_stall) ?
                          inst_spec_c1 : 4'b0000),
        .i_is_m_ext      (valid_c1 && is_m_ext_c1 && !wbq_cold_stall),
        .i_m_op          (m_op_c1),
    ...
    // Keep the physically local EX/LS/q0 wakeup path fast. q1 is the cold spill:
    // a q1 match only updates the held LS slot and becomes requestable next
    // cycle, removing q1 tag/data from the global ready and memory fast paths.
    assign ls_operands_ready =
        (!ls_addr_pending || addr_dep_ex_match || addr_dep_ls_match ||
         (addr_dep_q0_match && !addr_dep_q1_match)) &&
        (!ls_data_pending || data_dep_ex_match || data_dep_ls_match ||
         (data_dep_q0_match && !data_dep_q1_match));
    ...
    assign c1_busy      = m_busy_c1 || wbq_cold_stall ||
    ...
    bpu_top u_bpu_top (
        ...
        .i_rst_n         (rst_front_n_q),
        .i_pc_cur        (pc_p),
        .i_update_en     (c1_advance && update_en_c1),
