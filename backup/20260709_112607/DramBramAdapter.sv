...
module DramBramAdapter #(
    ...
    logic [3:0]            dram_we;
    logic [1:0]            read_offset_pipe [0:1];
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            resp_valid   <= 1'b0;
            read_valid_pipe <= 2'b00;
            read_offset_pipe[0] <= 2'd0;
            read_offset_pipe[1] <= 2'd0;
        end else begin
            read_valid_pipe <= {read_valid_pipe[0], req_valid && !req_write};
            resp_valid <= read_valid_pipe[1];
            if (req_valid && !req_write) begin
                read_offset_pipe[0] <= req_addr[1:0];
            end
            read_offset_pipe[1] <= read_offset_pipe[0];
        end
    end

    assign resp_rdata = dram_rdata_raw >> {read_offset_pipe[1], 3'b000};
endmodule
