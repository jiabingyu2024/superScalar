...
// - Miss refill issues four critical-word-first reads in consecutive cycles.
//   Fixed-order responses fill the line while the critical word restarts the
//   core, avoiding repeated payment of the registered memory latency.
// - The first cacheable miss request is issued from DC_REFILL one cycle after
//   lookup, cutting the tag-compare-to-external-memory combinational path.
//------------------------------------------------------------------------------
module DCache #(
    parameter int unsigned LINE_COUNT = 256,
    ...
    localparam int unsigned CACHE_ADDR_BITS = $clog2(CACHE_ADDR_END -
                                                     CACHE_ADDR_START);
    ...
    (* ram_style = "distributed" *) logic [31:TAG_LSB] tag_q [0:LINE_COUNT-1];
    ...
    logic [INDEX_W-1:0] data_write_index_bank_c [0:WORDS_PER_LINE-1];
    ...
    logic [1:0]  fill_req_word_q;
    logic [1:0]  fill_resp_word_q;
    logic [2:0]  fill_req_count_q;
    logic [2:0]  fill_resp_count_q;
    ...
    logic [31:0]        cache_word_visible_c;
    ...
    logic               store_update_pending_q;
    logic [INDEX_W-1:0] store_update_index_q;
    logic [1:0]         store_update_word_q;
    logic [31:0]        store_update_data_q;
    logic [3:0]         store_update_mask_q;


    generate
        for (genvar word_idx = 0; word_idx < WORDS_PER_LINE; word_idx++) begin : gen_data_word
            assign data_write_index_bank_c[word_idx] = data_write_index_c;
            for (genvar byte_idx = 0; byte_idx < 4; byte_idx++) begin : gen_data_byte
                DCacheDataByteBank #(
                    ...
                    .write_addr(data_write_index_bank_c[word_idx]),
    ...
    assign req_cacheable_c = !cpu_req_uncached &&
                             (cpu_req_addr[31:CACHE_ADDR_BITS] ==
                              CACHE_ADDR_START[31:CACHE_ADDR_BITS]);
    ...
    // A store update is written on the current edge.  Merge it into a
    // same-word load so the cache can still accept one request per cycle.
    always_comb begin
        cache_word_visible_c = cache_word_c;
        if (store_update_pending_q &&
            (store_update_index_q == req_index_c) &&
            (store_update_word_q == req_word_c)) begin
            for (int byte_idx = 0; byte_idx < 4; byte_idx++) begin
                if (store_update_mask_q[byte_idx]) begin
                    cache_word_visible_c[byte_idx*8 +: 8] =
                        store_update_data_q[byte_idx*8 +: 8];
                end
            end
        end
    end
    ...
    always_comb begin
        ...
        if (!rst) begin
            // Store-hit updates are registered before driving the distributed
            // RAM write port.  This removes tag lookup and LSU dependency
            // resolution from the RAM WE setup path.
            if (store_update_pending_q) begin
                data_write_en_c    = 1'b1;
                data_write_index_c = store_update_index_q;
                data_write_word_c  = store_update_word_q;
                data_write_data_c  = store_update_data_q;
                data_write_mask_c  = store_update_mask_q;
            end else if ((state_q == DC_REFILL) && mem_resp_valid) begin
                data_write_en_c    = 1'b1;
                data_write_index_c = miss_index_c;
                data_write_word_c  = fill_resp_word_q;
    ...
    always_comb begin
        ...
        unique case (state_q)
            DC_IDLE: begin
                if (cpu_req_valid) begin
                    if (cpu_req_write) begin
                        ...
                        // from DC_REFILL on the following cycle.
                        cpu_req_ready    = 1'b0;
                    end else begin
                        mem_req_valid    = 1'b1;
                        mem_req_write    = 1'b0;
            ...
            DC_REFILL: begin
                mem_req_valid    = fill_req_count_q < 3'(WORDS_PER_LINE);
                mem_req_write    = 1'b0;
                mem_req_addr     = {miss_addr_q[31:4], fill_req_word_q, 2'b00};
                mem_req_uncached = 1'b0;
    ...
    assign cpu_resp_valid = resp_valid_q ||
                            ...
                            ((state_q == DC_REFILL) && restart_pending_q);
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            fill_req_word_q <= 2'd0;
            fill_resp_word_q <= 2'd0;
            fill_req_count_q <= 3'd0;
            fill_resp_count_q <= 3'd0;
            resp_rdata_q <= 32'd0;
            resp_valid_q <= 1'b0;
            restart_pending_q <= 1'b0;
            store_update_pending_q <= 1'b0;
            store_update_index_q <= '0;
            store_update_word_q <= '0;
            store_update_data_q <= '0;
            store_update_mask_q <= '0;
            ...
            unique case (state_q)
                DC_IDLE: begin
                    if (cpu_req_valid) begin
                        if (cpu_req_write) begin
                            if (mem_req_ready) begin
                                if (req_cacheable_c) begin
                                    ...
                                    if (!req_hit_c) begin
                                        ...
                                        store_update_pending_q <= 1'b1;
                                        store_update_index_q <= req_index_c;
                                        store_update_word_q <= req_word_c;
                                        store_update_data_q <= store_shifted_data_c;
                                        store_update_mask_q <= store_shifted_mask_c;
                                    end
                                end
                            end
                        end else if (req_cacheable_c && req_hit_c) begin
                            perf_dcache_access <= perf_dcache_access + 64'd1;
                            resp_rdata_q <= cache_word_visible_c >> {cpu_req_addr[1:0], 3'b000};
                            resp_valid_q <= 1'b1;
                        end else if (req_cacheable_c) begin
                            perf_dcache_access <= perf_dcache_access + 64'd1;
                            perf_dcache_miss <= perf_dcache_miss + 64'd1;
                            ...
                            fill_req_word_q <= req_word_c;
                            fill_resp_word_q <= req_word_c;
                            fill_req_count_q <= 3'd0;
                            fill_resp_count_q <= 3'd0;
                            state_q <= DC_REFILL;
                ...
                DC_REFILL: begin
                    ...
                    if (mem_req_valid && mem_req_ready) begin
                        fill_req_count_q <= fill_req_count_q + 3'd1;
                        fill_req_word_q <= fill_req_word_q + 2'd1;
                    end

                    if (mem_resp_valid) begin
                        if (fill_resp_count_q == 3'd0) begin
                            resp_rdata_q <= fill_resp_shifted_c;
                            restart_pending_q <= 1'b1;
                        end

                        if (fill_resp_count_q == 3'(WORDS_PER_LINE - 1)) begin
                            tag_q[miss_index_c] <= miss_tag_c;
                            valid_q[miss_index_c] <= 1'b1;
                            fill_req_count_q <= 3'd0;
                            fill_resp_count_q <= 3'd0;
                            fill_req_word_q <= miss_target_word_q;
                            fill_resp_word_q <= miss_target_word_q;
                            state_q <= DC_IDLE;
                        end else begin
                            fill_resp_count_q <= fill_resp_count_q + 3'd1;
                            fill_resp_word_q <= fill_resp_word_q + 2'd1;
                        end
                    end
                end
...
`ifdef VERILATOR_TB
    always_ff @(posedge clk) begin
        if (!rst && (state_q == DC_REFILL)) begin
            if (fill_req_count_q > 3'(WORDS_PER_LINE)) begin
                $fatal(1, "DCache refill request counter overflow");
            end
            if (mem_resp_valid &&
                (fill_resp_count_q >= fill_req_count_q)) begin
                $fatal(1, "DCache response without an outstanding refill word");
            end
        end
    end
`endif
