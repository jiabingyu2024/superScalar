...
当前状态要先说清楚：本项目已经接入了独立 DiffTest harness，可以采集并自检 CPU commit trace。`dev-v0` 引用的 OpenXiangShan DiffTest submodule 没有迁入当前分支，因为它尚未生成可用接口，也没有接入 NEMU/Spike reference。因此当前结果 JSON 中：

当前 `dev-v1.1` 的适配器从 OoO core 的 ROB retire 边界采集两条 commit lane，并按 lane 0、lane 1 的架构顺序消费。`myCPU`/`rv32` 和 `student_top/src` harness 都已接通，且都使用当前 OoO core。

本文后续涉及 `tb/difftest/upstream` 或 reference model 的内容是 `dev-v0` 的历史设计说明，不适用于当前分支。
...
|---|---|---|---|
| `rv32` | `myCPU` | `myCPU.u_core` 的 ROB retire 边界 | ISA 小测试、快速定位指令级问题 |
| `src` | `student_top` | `student_top.Core_cpu.u_core` 的 ROB retire 边界 | 跑完整 src 程序，同时观察 CPU commit |
