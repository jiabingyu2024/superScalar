...
> 修复状态（2026-07-18）：已在 `rtl/core/memory/dcache.sv` 实施 victim-tag early invalidate；接口与 filelist 均未变化。本文末尾记录具体改动和待执行回归。
...
采用前述方案 1：在 `DC_IDLE` 检测到 cacheable load miss 的同一拍，通过现有 tag RAM 单写口把 victim 项写为全零：
...
```text
load miss 被确认
  -> victim valid 在任何 refill data response 到达前清零
  -> refill 中途被 kill：部分 data 可以残留，但该 index 不可 hit
  -> 下次访问旧行或新行都重新 miss/refill
  -> refill 正常完成：最后一拍原子安装新 tag/valid
```

该方案没有增加端口、状态或 data buffer，只在原 tag RAM 写口增加一个与 `DC_INIT`、最终 refill 安装互斥的写入来源。代价是 killed refill 后重访该 index 必然 miss，这是正确性优先且可接受的恢复行为。

本次只完成 RTL 定向修复和静态检查；按照增量 RTL 修复流程边界，尚未在本步骤内执行编译/仿真。后续必须依次执行：

```bash
make verilator-build-src BUILD_JOBS=4 BUILD_CXX=g++ OBJCACHE=
python3 scripts/run_verilator.py src --test src2 --max-cycles 4000000 --no-build
python3 scripts/run_verilator.py src --test srcSmoke --max-cycles 500000 --no-build
python3 scripts/run_verilator.py src --test srcWithMext --max-cycles 500000 --no-build
```

修复验收标准仍是：`src2` 跑过原 FAIL 点且不写 `0x24181824`，再完成完整 src 回归与 killed-refill directed test；不能只凭 `srcWithMext` 短窗口无 fail 判定修复完成。
