...
## 阶段 1/2 执行记录（2026-07-05）

用户要求按本计划完成阶段 1 和阶段 2。本轮已完成以下 RTL 改动：

| 阶段 | 方案 | 状态 | 修改文件 |
|---|---|---|---|
| 阶段 1 | 方案 1：MEM load return result buffer | 已完成 | `rtl/core/ExecuteStage/ExecuteMemStage.sv` |
| 阶段 1 | 方案 4：DRAM store fairness | 已完成 | `rtl/core/DramAccessIF.sv`, `rtl/core/core.sv` |
| 阶段 2 | 方案 2a：ROB 16→32 | 已完成 | `rtl/core/BasicTypes.sv` |
| 阶段 2 | 方案 3：MUL 预计唤醒校准 | 已完成（仅非 DIV/REM MUL） | `rtl/core/DispatchStage/DispatchStage.sv`, `rtl/core/DispatchStage/IssueQueue.sv` |

### RTL 实现摘要

1. `ExecuteMemStage.sv`
   - 新增 1-entry `memResultBuf`，保存已经 merge/extend 后的 `ExMemToWbPath`。
   - MEM→WB 输出优先级改为：旧 buffer 先出；当前 MEM uop 可继续占用空 lane；同周期 load return 若无空 lane 则写入 buffer。
   - `loadReturnBlocked` 不再由 `loadMetaPipe1.valid && currentMemValid` 触发，只在 load return 无法输出且 buffer 也不能接收时触发。
   - 结果：解除原先“load 返回 + 任意当前 MEM uop = 全 EX stall”的 0.5 IPC 限制。

2. `DramAccessIF.sv`
   - 新增 store aging 仲裁，`STORE_STARVE_LIMIT = 2`。
   - 连续 store commit 被 load read 抢占达到阈值后，下一次仲裁强制 store 优先。
   - `exReadReady` 只在实际 `readGrant` 时为 1；load 被 store aging 拒绝时，`ExecuteMemStage` 会通过 `loadAccessBlocked` 保持 load metadata 对齐。

3. `core.sv`
   - `store_commit_blocked_by_load_cycles` 改为统计仲裁后的真实 `readEn`，避免把已被 store aging 拒绝的 load 请求误算为读抢占。

4. `BasicTypes.sv`
   - `ROB_DEPTH` 从 16 扩到 32。ROB/FIFO index 宽度由 `ROB_DEPTH_WIDTH` 自动扩展。

5. `DispatchStage.sv` + `IssueQueue.sv`
   - 非 DIV/REM 的 MUL 预计唤醒从 3 拍校准为 4 个调度间隔，确保 consumer 到 EX 时 producer 已进入 WB bypass 可见窗口。
   - IssueQueue 预计唤醒白名单从仅 `delay == 1` 扩展为 `delay == 1` 或“非 DIV/REM MUL 的校准 delay”。
   - MEM load 预计唤醒暂未放开：阶段 1 result buffer 可能引入可变 1 拍返回排队，继续用真实 WB wakeup 更安全。
   - DIV/REM 预计唤醒暂未放开：34 拍 IP 输出相位和 backpressure 还未单独校准。
...
| 命令 | 结果 |
|---|---|
| `make verilator-build BUILD_JOBS=4 BUILD_CXX=g++ OBJCACHE=` | PASS |
| `make verilator-build-src BUILD_JOBS=4 BUILD_CXX=g++ OBJCACHE=` | PASS |

RV32 回归：

| 类别 | 测试 | 结果 |
|---|---|---|
| load/store | `rv32ui-p-lw/sw/lb/lh/lbu/lhu/sb/sh` | PASS |
| 基础提交/ALU/跳转 | `rv32ui-p-simple/add/sub/and/or/jal` | PASS |
| M 扩展 | `rv32um-p-mul/mulh/mulhu/mulhsu/div/rem` | PASS |
...
该命令仍按预期 TIMEOUT（200k 周期上限），但性能桶已经显示阶段收益：

| 指标 | 优化前 | 阶段 1/2 后 | 变化 |
|---|---:|---:|---:|
| commit_count | 99,534 | 153,160 | +53,626 |
| IPC | 0.4977 | 0.7658 | +53.9% |
| frontend_stall_cycles | 198,171 | 119,232 | -78,939 |
| rob_full_cycles | 148,405 | 168 | 基本消除 |
| issue_queue_full_cycles | 3 | 118,083 | 成为新瓶颈 |
| load_return_block_cycles | 98,297 | 0 | 消除 |
| load_access_block_cycles | 0 | 0 | 未引入 load 仲裁等待 |
| ex_stall_cycles | 98,297 | 0 | 消除 |
| store_commit_blocked_by_load_cycles | 15 | 4,743 | 上升，但未造成 StoreBuffer full 主导 |
| store_buffer_full_cycles | 12 | 68 | 仍很低 |
| issue w2 cycles | 555 | 39,121 | 双发明显恢复 |
| commit w2 cycles | 339 | 57,124 | 双提交明显恢复 |

当前新结论：

1. 阶段 1 命中第一瓶颈：`load_return_block_cycles` 和 `ex_stall_cycles` 均降为 0。
2. 阶段 2 后 ROB full 不再是主要瓶颈：`rob_full_cycles` 从 74.2% 降到 0.084%。
3. 新主瓶颈转移到 IssueQueue：`issue_queue_full_cycles = 118083`，后续不应继续优先扩大 ROB，而应优化 IQ 容量/选择/释放节奏或进一步减少长依赖占队列时间。
4. store fair 保护有效性需要继续观察：`store_commit_blocked_by_load_cycles` 因 load 连续性提高而上升，但 `store_buffer_full_cycles` 仍很低，当前未形成新的前端主阻塞。

---

## 各模块瓶颈详细分析（优化前基线证据）

以下 RTL 证据用于说明本轮优化前的 IPC < 0.5 根因。阶段 1/2 已完成后，部分代码行和控制条件已经被上方“阶段 1/2 执行记录”中的实现取代。
