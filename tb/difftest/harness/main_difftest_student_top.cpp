...
#include "../../verilator/dut_student_top_io.h"
...
int main(int argc, char** argv) {
    try {
        ...
        if (opt.mode != "src") {
            throw std::runtime_error("student_top difftest harness only supports --mode=src");
        }
        ...
        for (uint64_t cycle = 0; cycle < opt.max_cycles && !Verilated::gotFinish() &&
            ...
            mirror.tick_request(req, false, opt.counter_cycles_per_ms);
            for (const auto& commit : difftest::capture_commits(top)) {
                difftest.observe(now, commit, result);
            }
