...
module InOrderFetchStage(
    ...
    output PcPath iromAddr,
    input InstPath iromData,
    output logic iromEna,
    ...
    assign iromEna = run && !stall;
    assign iromAddr = reqPc;

    assign bpuReqTaken = bpuValid[bpu_index(reqPc)] &&
                         bpuTag[bpu_index(reqPc)] == bpu_tag(reqPc) &&
                         bpuCounter[bpu_index(reqPc)][1];
    assign bpuReqTarget = bpuReqTaken ? bpuTarget[bpu_index(reqPc)] : (reqPc + 32'd4);
