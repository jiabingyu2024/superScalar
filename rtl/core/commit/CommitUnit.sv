...
module CoreCommitUnit (
    ...
    output logic [1:0] csr_priv_mode_o,
    ...
    always_comb begin
        ...
        csr_priv_mode_o = priv_q;

        recover_valid_o = 1'b0;
        recover_pc_o = '0;
        timer_interrupt_take = timer_irq_i && mstatus_q[MSTATUS_MIE] &&
                               mie_q[7] && rob_valid_i[0];
        stop_retire = timer_interrupt_take;
        if (timer_interrupt_take) begin
        ...
        for (int i = 0; i < RETIRE_WIDTH; i = i + 1) begin
            ...
            commit_arch_o[i] = rob_entry_i[i].alloc_prd ? rob_entry_i[i].uop.rd : '0;
            commit_prd_o[i] = rob_entry_i[i].alloc_prd ? rob_entry_i[i].prd : '0;
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (timer_interrupt_take) begin
                mepc_q <= rob_entry_i[0].uop.pc;
                mcause_q <= 32'h8000_0007;
                mtval_q <= 32'b0;
                mstatus_q[MSTATUS_MPP_MSB:MSTATUS_MPP_LSB] <= priv_q;
            ...
            for (int i = 0; i < RETIRE_WIDTH; i = i + 1) begin
                ...
                if (trap_take[i]) begin
                    ...
                    mstatus_q[MSTATUS_MIE] <= 1'b0;
                    priv_q <= PRIV_M;
                end else if (rob_ready_o[i] && rob_entry_i[i].uop.is_mret) begin
                    priv_q <= mstatus_q[MSTATUS_MPP_MSB:MSTATUS_MPP_LSB];
