...
module CoreMulDivIssueQueue (
    ...
    input  logic [MULDIV_ISSUE_WIDTH-1:0] issue_ready_i,
    output logic [MULDIV_ISSUE_WIDTH-1:0] issue_valid_o,
    output CoreRenamedUop [MULDIV_ISSUE_WIDTH-1:0] issue_uop_o
);
    CoreCompressedQueue #(
        .DEPTH(MULDIV_IQ_DEPTH),
        .ISSUE_PORTS(MULDIV_ISSUE_WIDTH),
        .HEAD_ONLY(1'b1)
    ) u_queue (
        .clk(clk),
        .rst(rst),
